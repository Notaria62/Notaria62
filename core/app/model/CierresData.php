...
class CierresData
{
    ...
    {
        $destino = addslashes($this->destino);
        $sql = "UPDATE " . self::$tablename . " SET nrocopia=\"$this->nrocopia\",is_registro=$this->is_registro,is_sustituto=$this->is_sustituto,dateescritura=\"$this->dateescritura\",is_ejemplar=$this->is_ejemplar,nroescriturapublica=\"$this->nroescriturapublica\", numfolios=$this->numfolios,observationcopy1=\"$this->observationcopy1\",observationcopy2=\"$this->observationcopy2\", destino=\"$destino\", notario_id=\"$this->notario_id\", user_id=\"$this->user_id\" WHERE id=$this->id ";
        Executor::doit($sql);
    }
