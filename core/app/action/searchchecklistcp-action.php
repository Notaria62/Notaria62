...
if (count($result)>0) {
    ...
    foreach ($result as $value) {
        ...
        $btnDel = ($us->user_level)?'<a href="./?action=admindelchecklists&checklists_id='.$value->checklists_id.'&nep='.$value->numeroescriturapublica.'&ep_anho='.$value->ep_anho.'" data-toggle="tooltip" title="Eliminar" class="btn btn-link btn-danger btn-just-icon btn-sm"> <i class="material-icons">delete</i></a>':'';
        $btnPrint = '<a onclick="openWindowsPrint(\'./?view=printcontrolofprocess&nep='.$value->numeroescriturapublica.'&anho='.$value->ep_anho.'&idcp='.$value->id.'&checklists_id='.$value->checklists_id.'\')"
        ...
        'options'=>$btnPrint.'<a href="./?view=editcontrolofprocess&nep='.$value->numeroescriturapublica.'&anho='.$value->ep_anho.'&idcp='.$value->id.'&checklists_id='.$value->checklists_id.'" data-toggle="tooltip" title="Editar" class="btn btn-link btn-success btn-just-icon btn-sm"><i class="material-icons">edit</i> </a>'.$btnDel.$btnModal
        );
    }
    echo json_encode($ar);
}